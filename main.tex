...
% --- Hyperlinks (Load this last!) ---
\usepackage[
    colorlinks=true,
    urlcolor=blue,
    citecolor=black,
    linkcolor=black,
    pdftitle={Your Document Title}, % Optional: Adds metadata to the PDF
    pdfauthor={Your Name}           % Optional: Adds metadata to the PDF
]{hyperref}
...
{\Large \textbf{Predictive Machine-Learning of Dynamic Interfacial Motifs Governing Plastic Response}}\\
...
These processes control Hall-Petch strengthening, back stress hardening, and ductility \cite{Cordero2016,Gao2022,LeBauschinger2018}.
...
To extend the training signal into the GB core, we add weak supervision from the local plastic strain field: using per-atom elastic constants \cite{Krief2024}, we compute the local plastic strain (total minus elastic) and train the CNN-predicted activity to be consistent with it, inspired by the field-matching loss of Bertin \emph{et al.}\ \cite{Bertin2024}.
Crucially, supervision targets dislocation \emph{motion} events rather than static defect positions: DXA labels from consecutive frames are differenced to identify voxels where dislocations have moved, so the CNN learns to recognise dynamic interfacial motifs---the correlated displacement signatures of active slip---rather than merely cataloguing lattice distortion.
...
All data are classical molecular dynamics (MD) atomic trajectories, built on the group's simulation pipeline \cite{Krief2021,Krief2024}, which is based on massively parallel large scale MD simulations via the LAMMPS code \cite{plimpton1995fast}.
...
\hline
\multicolumn{4}{|l|}{Total: ${\sim}\!20$ runs, ${\sim}\!300$\,GiB, ${\sim}\!300$ core-h MD (CPU cluster, ${\leq}\!2048$ cores) $ + $ ${\sim}\!100$ GPU-h (CNN)} \\
\hline
\end{tabular}
\end{center}

\noindent Each run yields ${\sim}10^3$ frames; with $10^5$--$10^6$ atoms per frame the labelled training set contains ${\sim}10^{8\text{--}9}$ per-atom observations. The campaign size may be adjusted based on validation-loss convergence.

\noindent Computational resources will be provided through the PI's \href{http://icpl.huji.ac.il}{ICPL} membership; no compute budget requested.
...
\textbf{PI:} Dr.\ Menahem Krief (computational materials science, atomistic simulations). Krief developed methods for local evaluation of elastic constants from MD \cite{Krief2021} and mapped elastic heterogeneity across nanocrystalline GBs \cite{Krief2024}; the same feature-extraction infrastructure and Cu pipeline are reused here.
...
\end{itemize}
CPU cluster and GPU time provided by \href{http://icpl.huji.ac.il}{ICPL}; no compute budget requested.

\section*{Future plans}

This 12-month foundational phase establishes the computational architecture and empirical validation necessary to bridge atomistic simulations with predictive machine learning. We aim to deliver: a documented MD-to-ML pipeline, trained CNN models for Cu bicrystals, a precision/recall benchmark against DXA, and proof-of-concept GB activity maps.
